...
import { Ticket, Search, Filter, ArrowRight, ListFilter } from 'lucide-react';
...
export const TicketQueuePage = () => {
  ...
  const filteredTickets = DUMMY_TICKETS.filter(t => {
    ...
    const matchesFilter = filter === 'All' || 
                        (filter === 'Priority' ? t.isPriority : t.priority === filter);
    return isPrivate && matchesFilter;
  });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="px-4 py-2 bg-red-100 text-red-700 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center gap-2">
            ...
            {DUMMY_TICKETS.filter(t => t.isPriority && t.isPrivate).length} Perlu Prioritas
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          ...
          {filteredTickets.length === 0 && (
            <div className="p-20 flex flex-col items-center text-center">
              ...
              <p className="text-slate-500 text-sm max-w-xs">Tidak ada keluhan yang perlu ditangani saat ini.</p>
            </div>
          )}
        </div>
      </div>
